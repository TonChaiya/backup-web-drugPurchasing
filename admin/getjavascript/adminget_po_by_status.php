    ...
    try {
        ...
        if ($records) {
            ...
            foreach ($records as $record) {
                echo '<tr data-po-number="' . htmlspecialchars($record['po_number']) . '">';  // เพิ่ม data-po-number
                echo '<td class="py-2 px-4 border-b">';
                echo htmlspecialchars($record['po_number']) . ' ';
                echo '<a href="../admin/generate_pdfforgporequest.php?po_number=' . urlencode($record['po_number']) . '" target="_blank" class="text-blue-500 hover:underline ml-2">View</a>';
                echo ' | ';
                echo '<a href="../admin/download_pdf.php?po_number=' . urlencode($record['po_number']) . '" class="text-green-500 hover:underline ml-2">Download</a>';
                echo '</td>';
