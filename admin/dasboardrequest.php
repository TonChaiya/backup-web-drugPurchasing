...
<body>
    ...
    <div id="modal" class="fixed inset-0 hidden bg-gray-800 bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-3xl">
            ...
            <button id="downloadAll" class="mt-4 p-2 bg-blue-500 text-white rounded hover:bg-blue-600">ดาวน์โหลดใบเบิกทั้งหมด</button>
        </div>
    </div>
    ...
    <script>
        function openModal(status) {
            ...
            // ฟังก์ชันดาวน์โหลดทั้งหมด
            document.getElementById("downloadAll").onclick = function() {
                var poNumbers = [];
                var rows = document.querySelectorAll("tr[data-po-number]"); // ใช้ data-po-number เพื่อดึงแถวที่มีใบเบิก
                rows.forEach(function(row) {
                    var poNumber = row.getAttribute("data-po-number"); // ดึงหมายเลขใบเบิกจาก data-po-number
                    if (poNumber) {
                ...
                if (poNumbers.length === 0) {
                    console.log("ไม่มีใบเบิกที่จะดาวน์โหลด");
                    alert("ไม่พบใบเบิกที่จะดาวน์โหลด");
                    return;
                }

                console.log("ใบเบิกที่จะดาวน์โหลด: ", poNumbers);

                // สร้าง URL สำหรับดาวน์โหลดไฟล์ PDF ของหลายใบเบิก
                var downloadLinks = poNumbers.map(function(poNumber) {
                    return "../admin/download_pdf.php?po_number=" + encodeURIComponent(poNumber);
                });
